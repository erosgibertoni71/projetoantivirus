...
def buscar_arquivos_gui(entry_diretorio, lista_arquivos, check_exe, check_bat):
    """
    ...
    # Determina as extensões a serem buscadas com base nos checkboxes
    extensoes = []
    if check_exe.get():
        extensoes.append(".exe")
    if check_bat.get():
        extensoes.append(".bat")
    if not extensoes:
        messagebox.showinfo("Nenhum Tipo Selecionado", "Selecione pelo menos uma extensão para buscar.")
        return

    arquivos_encontrados = buscar_arquivos(diretorio, tuple(extensoes))
    ...
    if arquivos_encontrados:
        ...
        messagebox.showinfo("Nenhum Arquivo Encontrado", "Nenhum arquivo com as extensões selecionadas foi encontrado.")
...
def main():
    ...
    frame_tipos.pack(pady=10, fill=tk.X, padx=10)

    label_tipos = tk.Label(frame_tipos, text="Tipos de Arquivos:")
    label_tipos.pack(side=tk.LEFT, padx=5)

    check_exe = tk.BooleanVar()
    check_bat = tk.BooleanVar()

    checkbox_exe = tk.Checkbutton(frame_tipos, text=".exe", variable=check_exe)
    checkbox_exe.pack(side=tk.LEFT, padx=5)

    checkbox_bat = tk.Checkbutton(frame_tipos, text=".bat", variable=check_bat)
    checkbox_bat.pack(side=tk.LEFT, padx=5)
    ...
    btn_buscar = tk.Button(frame_botoes, text="Buscar Arquivos",
                           command=lambda: buscar_arquivos_gui(entry_diretorio, lista_arquivos, check_exe, check_bat))
